---
name: lark-wiki
description: "LarkSuite Wiki knowledge base management — spaces, nodes, pages, members, search.
Use when user asks about wiki, knowledge base, space, page, node, team wiki, documentation,
create page, search docs, manage wiki members. Requires lark-token-manager MCP."
---
...
```
Use `lark_open_id` as `member_id` for `add_member` / `delete_member` with `member_type="openid"`.
