...
### Task (`task-v2`)
**Scopes**: `task:task:read`, `task:task:write`, `task:tasklist:read`, `task:tasklist:write`
**Resources**: Task, Subtask, Tasklist, Section, Comment, CustomField, Attachment
**Key ops**: CRUD tasks/lists, manage members, dependencies, reminders, custom fields
...
### Contact (`contact-v3`)
**Scopes**: `contact:user.base:readonly`, `contact:user.id:readonly`, `contact:contact.base:readonly`
**Resources**: User, Department, Group, FunctionalRole, CustomAttr, EmployeeTypeEnum
**Key ops**: Lookup users/departments, search org directory, manage groups
**Docs path**: `docs/contact-v3/`

### IM - Instant Messaging (`im-v1`)
**Scopes**: `im:message`, `im:message:readonly`, `im:chat`, `im:chat:readonly`
**Resources**: Message, Chat, Image, File, Pin, Reaction, BatchMessage, Card
**Key ops**: Send/receive messages, manage chats, upload images/files, message cards
...
**Key ops**: CRUD files/folders, upload/download, manage permissions, search
